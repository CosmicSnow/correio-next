          ...
          {messages ? (
            ...
            >
              {messages.map((message) => (
                <button className="appearance-none text-left" key={message.id}>
                  <Message content={message.content} bgColor={message.color} />
                </button>
              ))}
            </div>
          ) : (
            <div className="w-full h-64 flex items-center justify-center flex-col">
              ...
              <span className="mt-2 dark:text-gray-300 text-gray-700 text-xl px-8">
                Estamos carregando suas mensagens. Caso elas não apareçam, tente
                sair da sua conta e entrar denovo.
              </span>
            </div>
          )}
        </div>
